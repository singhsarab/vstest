...
namespace Microsoft.VisualStudio.TestPlatform.CrossPlatEngine
{
    ...
    /// </summary>
    internal class AppDomainTestInvoker<T> : ITestInvoker where T : MarshalByRefObject, ITestInvoker, new()
    {
        private const string XmlNamespace = "urn:schemas-microsoft-com:asm.v1";

        protected readonly AppDomain appDomain;

        protected readonly ITestInvoker actualInvoker;

        private string mergedTempConfigFile = null;
    
        public AppDomainTestInvoker(string testSourcePath)
        {
        ...
        { 
             try
            {
               actualInvoker.InvokeRun(pathToAdditionalExtensions, sourceMap, runsettings, testExecutionContext, handler);
            }
            finally
            {
                try
        ...
        internal AppDomain CreateNewAppDomain(string testSourcePath)
        {
    ...
    internal void AddAssemblyResolver(AppDomain appDomain)
    {
    // Create Custom assembly resolver in new appdomain before anything else to resolve testplatform assemblies
            appDomain.CreateInstanceFromAndUnwrap(
                    typeof(CustomAssemblyResolver2).Assembly.Location,
                    typeof(CustomAssemblyResolver2).FullName,
                    false,
        ...
        /// <summary>
        /// Create the Invoker in new AppDomain based on test source path
        /// </summary>
        ...
        /// <returns></returns>
        private ITestInvoker CreateInvokerInAppDomain(AppDomain appDomain)
        {
            AddAssemblyResolver(appDomain);
            
            // Create Invoker object in new appdomain
            var InvokerType = typeof(T);
            return (ITestInvoker) appDomain.CreateInstanceFromAndUnwrap(
    ...
    /// </summary>
    internal class CustomAssemblyResolver2 : MarshalByRefObject
    {
        private readonly IDictionary<string, Assembly> resolvedAssemblies;

        private readonly string[] resolverPaths;

        public CustomAssemblyResolver2(string testPlatformPath)
        {
